...
int main(int argc, char **argv)
{
	if (argc == 1)
	{
		printf("\nERROR:\n\n\tprogram must be used like this: ./main.exe <number_photos> .i.e.: ./main.exe 100\n\n");
		return 1;
	}
	...
	while (true)
	{
		...
		{
			system("ffmpeg -start_number 0 -framerate 30 -i pic%04d.jpeg -c:v libx264 -profile:v high -crf 20 -pix_fmt yuv420p _v_.mp4 && rm -rf pic*.jpeg");
			system("ffmpeg -i _v_.mp4 -filter:v \"setpts=4.2*PTS\" output.mp4 && rm -rf _v_.mp4");
			//system("ffmpeg -i _v_.mp4 output.mp4 && rm -rf _v_.mp4");
			break;
		}
